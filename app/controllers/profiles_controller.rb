class ProfilesController < ApplicationController
  def show
    @user = current_user
    @posts  = current_user.posts.includes(:user, :post_videos, :categories).order(created_at: :desc)
    @bookmark_posts = current_user.bookmark_posts.includes(:user, :post_videos, :categories).order(created_at: :desc)
